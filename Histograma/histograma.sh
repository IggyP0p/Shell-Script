...
funcao_contaNumero(){
	...
	#teste de leitura de arquivo
        ...
        #Pegando os números e listando com a quantidade
	echo -e "\n----------------------------------------------------------------"
        while [ "$N2" != "$NUMERO" ];
        do
        	cont=$(( $cont + 1 ))
		...
		#Decide se deve imprimir os que menos aparecem ou os que mais aparecem
		if [ "$3" = "-i" ];
                then
		...
		#Recorta o número da frequência e o número em si
		NUM=$(echo $VALOR | awk -F " " '{ print $1 }')
                NUMERO=$(echo $VALOR | awk -F " " '{ print $2 }')
		#Limita o número de números unicos
		[[ -z $2 ]] || if [[ "$cont" -gt "$2" ]];then break;fi
...
funcao_contaPalavra(){
	...
	do
		...
		#Limita o número de palavras unicas
		[[ -z $2 ]] || if [ "$cont" -gt "$2" ];then break;fi
...
funcao_contaLetra(){
	...
	echo -e "\n----------------------------------------------------------------"
		...
		#Limita o número de letras unicas
		[[ -z $2 ]] || if [ "$cont" -gt "$2" ];then break;fi
...
Projeto de Igor Barbosa de Sousa. Esse shell script foi feito para gerar uma tabela com dados e a frequência em que esses dados
